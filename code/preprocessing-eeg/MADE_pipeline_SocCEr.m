...
function [] = MADE_pipeline_SocCEr_oldEEGlab(dataset, subjects, session)
...
parfor file_locater_counter = 1:length(subjects_to_process) %1:4
		...
		desc = '';
            ...
            % Marlene 2025 I need to review badICs manually!!
            if save_interim_result==1
                try
                    badICs = adjusted_ADJUST(EEG_copy, [[output_location filesep] strrep(datafile_names{subject}, ext, '_adjust_report')]);
                catch ME
                    fprintf('ADJUST failed with: %s', ME.message);
                    % try to extract any partial results that ADJUST might have saved
                    adjust_report_file = [[output_location filesep] strrep(datafile_names{subject}, ext, '_adjust_report.txt')];
                    if exist(adjust_report_file, 'file')
                        % parse the partial results from the report file
                        badICs = parse_partial_adjust_results(adjust_report_file);
                    else
                        badICs = [];
                    end
                end
