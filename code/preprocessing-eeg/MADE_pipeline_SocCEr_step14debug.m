...
function [] = MADE_pipeline_SocCEr(dataset, subjects, session)
...
% Adjustment for SocCEr dataset (a cooperation between the NCDLab and KU Eichstätt-Ingolstadt) by Marlene Buch, 2025
% ************************************************************************
...
main_dir = "C:/Users/localadmin/Documents/08_SocCEr/soccer-dataset"; %local directory
...
% addpath(genpath('/home/data/NDClab/tools/lab-devOps/scripts/MADE_pipeline_standard/eeg_preprocessing'));% HPC: enter the path of the folder in this line
addpath(genpath('C:/Matlab-Stuff'));% local: enter the path of the folder in this line
...
rawdata_location_parent = fullfile(main_dir, 'sourcedata', 'checked', session, 'eeg');
rawdata_location_parent = char(rawdata_location_parent);

% 2. Enter the path of the channel location file
% channel_locations = loadbvef('/home/data/NDClab/datasets/soccer-dataset/code/soccer_eeg/chan_locs_files/electrode_locs_files/CACS-128-X7-FIXED-64only.bvef');
...
% channel_locations = loadbvef('/home/data/NDClab/tools/lab-devOps/scripts/MADE_pipeline_standard/eeg_preprocessing/chan_locs_files/electrode_locs_files/CACS-128-X7-FIXED-64only.bvef'); % HPC
channel_locations = loadbvef('C:/Matlab-Stuff/CACS-128-X7-FIXED-64only.bvef'); %local


%% STIMULUS TRIGGERS
% trial event triggers
fixation_triggers = {'S  1'};  % fixation cross onset **TIMING-CRITICAL: trial start marker**
stimulus_triggers = {'S  2'};  % stimulus array onset **TIMING-CRITICAL: stimulus onset marker**
mask_triggers = {'S  4'};  % mask onset (at SMI frame) **TIMING-CRITICAL: mask onset marker**

% trial information triggers
target_letter_triggers = {'S 16', 'S 17', 'S 18', 'S 19', 'S 20', 'S 21'};  % target letter (P=16, W=17, M=18, V=19, B=20, R=21)
flanker_letter_triggers = {'S 32', 'S 33', 'S 34', 'S 35', 'S 36', 'S 37'};  % flanker letter (P=32, W=33, M=34, V=35, B=36, R=37)
visible_trigger = {'S112'};  % target visible (SMI > 0)
invisible_trigger = {'S113'};  % target invisible (SMI = 0)

% other event triggers
tooslow_trigger = {'S  5'};  % too slow (deadline exceeded) **TIMING-CRITICAL: deadline marker**
confidence_prompt_trigger = {'S 64'};  % confidence prompt shown **TIMING-CRITICAL: confidence onset marker**

% block setup triggers
social_trigger = {'S 96'};  % social condition
nonsocial_trigger = {'S 97'};  % non-social condition
feedback_trigger = {'S  6'};  % block feedback displayed **TIMING-CRITICAL: feedback onset marker**

%% RESPONSE TRIGGERS
% key response triggers
response_key_triggers = {'S 40', 'S 41', 'S 42'};  % s key=40, d key=41, f key=42 **TIMING-CRITICAL: response onset marker**

% confidence report trigger
confidence_response_trigger = {'S127'};  % confidence response submitted **TIMING-CRITICAL: confidence response marker**

% response classification triggers
correct_trigger = {'S  7'};  % correct response
vis_flanker_error_trigger = {'S  8'};  % visible flanker error
invis_flanker_error_trigger = {'S  9'};  % invisible flanker error
nonflanker_error_trigger = {'S 10'};  % no-flanker error
nogo_error_trigger = {'S 11'};  % no-go error
multiple_key_trigger = {'S 12'};  % multiple key responses
backup_trigger = {'S 13'};  % backup trigger (logic failure)
...
task_event_markers = response_key_triggers; % enter all the event/condition markers (i.e., stim + resp markers)
...
remove_baseline = 1; % 0 = NO (no baseline correction), 1 = YES (baseline correc    tion)
...
% parfor file_locater_counter = 1:length(subjects_to_process) %1:4
        ...
        % if ~isdir(rawdata_location) % HPC: isdir I Marlene 2025
        if ~isfolder(rawdata_location) % local: isfolder I Marlene 2025
        ...
        % datafile_names=datafile_names(~ismember({datafile_names.name},{'.', '..', '.DS_Store'})); % HPC only!, not needed for local version 2023b I Marlene 2025
        ...
        % [~, vhdr_file, ~] = fileparts(subjects_to_process(file_locater_counter)); % HPC I Marlene 2025
        [~, vhdr_file, ~] = fileparts(char(subjects_to_process(file_locater_counter))); % local I Marlene 2025
        if exist([output_location filesep 'MADE_logfiles'], 'dir') == 0
            mkdir([output_location filesep 'MADE_logfiles'])
        end
        % dfile=fullfile(output_location, 'MADE_logfiles', [char(vhdr_file) '_' string(datetime('now','Format','MM-dd-yyyy_HH-mm-ss')) '.log']); % HPC I Marlene 2025
        dfile=fullfile(output_location, 'MADE_logfiles', strcat(vhdr_file, '_', string(datetime('now','Format','MM-dd-yyyy_HH-mm-ss')), '.log')); % local I Marlene 2025        
        diary(dfile)
		...
		desc = '';
            ...
            %% STEP 1c: remove practice trial events from continuous data
            % practice trials occur before first block condition trigger
            block_condition_triggers = {'S 96', 'S 97'};
            block_indices = find(ismember({EEG.event.type}, block_condition_triggers));

            if ~isempty(block_indices)
                first_test_block_time = EEG.event(block_indices(1)).latency;

                % count S  1 trial onsets before and after removal
                s1_events_before = sum(strcmp({EEG.event.type}, 'S  1'));

                % remove all events before first test block
                practice_event_indices = find([EEG.event.latency] < first_test_block_time);

                if ~isempty(practice_event_indices)
                    % count S  1 events that will be removed (practice trials)
                    practice_s1_events = sum(strcmp({EEG.event(practice_event_indices).type}, 'S  1'));

                    fprintf('removing %d practice events (%d trials) before first block trigger\n', ...
                        length(practice_event_indices), practice_s1_events);

                    EEG.event(practice_event_indices) = [];
                    EEG = eeg_checkset(EEG);
                end

                % also trim the actual data before first test block
                first_sample = round(first_test_block_time);
                EEG.data = EEG.data(:, first_sample:end);
                EEG.pnts = size(EEG.data, 2);
                EEG.xmax = (EEG.pnts - 1) / EEG.srate;

                % adjust all event latencies
                for e = 1:length(EEG.event)
                    EEG.event(e).latency = EEG.event(e).latency - first_sample + 1;
                end
                EEG = eeg_checkset(EEG);

                % count remaining S  1 trial onsets (test trials)
                s1_events_after = sum(strcmp({EEG.event.type}, 'S  1'));

                fprintf('remaining test trials: %d (expected: 864)\n', s1_events_after);

                if s1_events_after == 864
                    fprintf('correct number of test trials (epochs) remaining\n');
                else
                    fprintf('unexpected number of test trials (missing %d)\n', 864 - s1_events_after);
                end

            else
                warning('no block condition triggers found - cannot identify practice/test boundary');
            end

            %% STEP 1d: add info from add. (not timing-event-timed) triggers I Marlene 2025, SocCer-specific
            EEG = extract_trial_info(EEG);
            ...
            %% Marlene 2025 I debug start
            % diagnostic script to test findpeaks path conflict
            % add this code right before line 875 in MADE_pipeline_SocCEr.m

            % Marlene 2025 I fix chronux findpeaks path conflict before ICA artifact detection
            chronux_findpeaks = which('findpeaks');
            if contains(chronux_findpeaks, 'chronux')
                rmpath(fileparts(chronux_findpeaks));
                fprintf('removed chronux findpeaks from path, using matlab signal processing toolbox version\n');
            end
            ...
            %% STEP 12: Segment data into fixed length epochs & add beh-info
            ...
            if epoch_data==1
                if task_eeg ==1 % task eeg
                    ...
                    % add trial_info to epochs 
                    EEG = add_trial_info_to_epochs(EEG);
                    ...
                    if exist(beh_file, 'file')
                        ...
                        EEG = integrate_behavioral_data(EEG, BEH);
                    else
                        warning('behavioral file not found: %s', beh_file);
                    end
